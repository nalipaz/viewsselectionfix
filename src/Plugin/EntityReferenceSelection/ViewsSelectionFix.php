...
/**
 * @file
 * Contains \Drupal\ViewsSelection_fix\Plugin\EntityReferenceSelection.
 */

namespace Drupal\viewsselectionfix\Plugin\EntityReferenceSelection;
 ...
 *   id = "views",
 *   label = @Translation("Views: Filter by an entity reference view (fixed)"),
 *   group = "views",
 *   weight = 1
 * )
 */
class ViewsSelectionFix extends PluginBase implements SelectionInterface, ContainerFactoryPluginInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    if ($options) {
      ...
      $form['view']['view_and_display'] = array(
        '#type' => 'select',
        '#title' => $this->t('View used to select the entities (fixed)'),
        '#required' => TRUE,
        '#options' => $options,
  ...
  public function getReferenceableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
    ...
    if ($result) {
      foreach ($result as $row) {
        ...
        $fields = array();
        foreach ($row['#options']['inline'] as $field) {
          if ($entity->$field->value) {
            $value = $entity->$field->value;
          }
          else if ($entity->$field->target_id){
            $term_id = $entity->$field->target_id;
        ...
        $label = implode($row['#options']['separator'], $fields);
